...
// BEGIN FF_TREE
  ...
  for(j=0;j<nsse;j++){
	  result[0] += sumup(&(formfactors.Ctree->c[j]), &(formfactors.Ctree->c[j]));
  }
// END FF_TREE
...
// BEGIN FF_LOOP
  ...
  ENDLOOP(Gen6)
  for(j=0;j<nsse;j++){
	  result[1] += 2*sumup(&(formfactors.Cloop->c[j]), &(formfactors.Ctree->c[j]));
  }
...
// BEGIN HEL_LOOPS
  ...
  /*v2complex test;*/
  /*test.v = F8;*/
  /**//*test.v = ChainB1(Spinor(1,2),0,ec(5),0,Spinor(3,1));*/
  /*if(v==1){*/
  /*fprintf(stderr,"%.20f\n",Re(test.c[0]));*/
  /*fprintf(stderr,"%.20f\n",Im(test.c[0]));*/
  /*exit(0);*/
